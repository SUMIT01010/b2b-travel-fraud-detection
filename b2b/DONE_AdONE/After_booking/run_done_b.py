...
def parse_args():
    ...
    parser.add_argument('--config', nargs='?', default=os.path.join(SCRIPT_DIR, 'config_done_b'), help='Input config path')
    return parser.parse_args()

def load_config(fname=None):
    if fname is None:
        fname = os.path.join(SCRIPT_DIR, 'config_done_b')
    
    # Resolve relative path if not absolute
    if not os.path.isabs(fname):
        possible_path = os.path.join(SCRIPT_DIR, fname)
    ...
    for key in ['cont_file', 'struc_file']:
        if key in config and not os.path.isabs(config[key]):
            config[key] = os.path.abspath(os.path.join(SCRIPT_DIR, config[key]))
...
def trainer(sess, model, x_train, y_train, num_epochs):
    with sess.as_default():
        ...
        emb_dir = os.path.join(SCRIPT_DIR, 'emb_b')
        os.makedirs(emb_dir, exist_ok=True)
        np.savetxt(os.path.join(emb_dir, config['experiment_name'] + '.emb'), final)

        print('Saving outlier values...')
        ovals_dir = os.path.join(SCRIPT_DIR, 'ovals_b')
        os.makedirs(ovals_dir, exist_ok=True)
        fname = os.path.join(ovals_dir, config['experiment_name'] + '-oval1')
        np.savetxt(fname, o1)
        fname = os.path.join(ovals_dir, config['experiment_name'] + '-oval2')
        np.savetxt(fname, o2)
        fname = os.path.join(ovals_dir, config['experiment_name'] + '-oval3')
        np.savetxt(fname, o3)
...
log_dir = os.path.join(SCRIPT_DIR, 'log_b/')
summ_file = os.path.join(log_dir, config['experiment_name'])
os.makedirs(summ_file, exist_ok=True)
...
from analyze_fraud_performance_b import analyze_performance
analyze_performance()
