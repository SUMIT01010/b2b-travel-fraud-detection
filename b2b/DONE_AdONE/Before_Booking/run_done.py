...
def parse_args():
    ...
    parser.add_argument('--config', nargs='?', default=os.path.join(SCRIPT_DIR, 'config_done'), help='Input config path')
    return parser.parse_args()

def load_config(fname=None):
    if fname is None:
        fname = os.path.join(SCRIPT_DIR, 'config_done')
...
def trainer(sess, model, x_train, y_train, num_epochs):
    with sess.as_default():
        ...
        ovals_dir = os.path.join(SCRIPT_DIR, 'ovals')
...
from analyze_fraud_performance import analyze_performance
analyze_performance()
